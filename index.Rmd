...
#### Alignment and MEDIPS metrics (ordered plots) 
...
#### Alignment and MEDIPS metrics (boxplots and statistical analysis) 
...
renderPlot({
  ...
  plt <- ggplot(genome_wide_reads_data) +
    geom_boxplot(aes(x = group.x, y = pctMapping)) +
    theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
    facet_grid(.~genomic_site.x, space = "free", scales = "free") + ylim(0,100) + ylab("% reads mapping") +
    xlab("Genomic Sites")
  if (input$`scaleYm`){
    plt <- plt + scale_y_log10()
  }
...
Tested the examples provided in `spiky`. 
\
Could not get to work a few functions. [Issue created on the GitHub repo](https://github.com/trichelab/spiky/issues/21)


##### this work in pending
...
pending with normalized counts
...
inputPanel(
  ...
  checkboxInput("scaleX3", label= "scale X axis",
                  value = FALSE, width = NULL),
  checkboxInput("normalizeCount", label= "Normalize counts",
                  value = FALSE, width = NULL)
)
...
# input bins 300bp, shore, shelf, island, inter, promoter
...
#### Bins specific for a protcol of interest
\
## this work in pending
\
For this analysis we have filtered out all bins with 0 coverage across all libraries.
\
Eliminate highly correlated features across all 25 samples 
\
group1 to identify highly correlated features (> 0.8)
\
group2 to identify uncorrelated features (< 0.5)
\
Check for distinct features/bins in group1 = intersect(2 and 3) 
\ 
histogram for their counts
```{r, echo=F, warning=F}
# inputPanel(
#   selectInput("protocol3", label = "Experimental Protocol",
#               choices = c("protocol3"), selected = "None", width = 2000),
#   sliderInput("corr1", label = "similarity",
#               min = 0, max = 1, value = 0.9, step = 0.01),
#   sliderInput("corr2", label = "dissimilarity",
#               min = 0, max = 1, value = 0.5, step = 0.01),
#   checkboxInput("scaleXn", label= "scale X axis",
#                   value = FALSE, width = NULL)
# )
# 
# observe({
...
#     session,
#     "protocol3",
#     choices = unique(ant$group)
#   )
# })


# reactive function to get the ycounts (not in selected protocol)
